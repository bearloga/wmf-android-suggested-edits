...
revert_status_data <- readr::read_csv("revert_status_data_title-descriptions.csv.gz", col_types = "DTiiccli")

# fetch archived revs to remove them from revert status dataset
...
WHERE ct_tag_id = 14 -- android app edit", "wikidatawiki")
...
rs_df %>%
  ...
  readr::write_csv("revert_status_data_title-descriptions.csv")
system("gzip --force revert_status_data_title-descriptions.csv") # compress
